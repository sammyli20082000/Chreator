...
public class ChessBoardPanel extends JPanel {
	...
	public static class CustomListCellRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
			...
			if (index < colorList.size())
				c.setBackground(colorList.get(index));
			return c;
		}
	...
	private JRadioButton imageFromFileRadio, blankImageRadio, addSinglePointRadio, addRectGridPointRadio,
			addTriGridPointRadio, scalePointNoneRadio, scalePointAllPanelRadio, scalePointAllImageRadio,
			scalePointCoordinatesPanelRadio, scalePointCoordinatesImageRadio;
	private JButton browseBoardImageButton, addDirectionButton, deleteDirectionButton, createEdgeForPointsButton,
			deleteEdgeForPointsButton, addPointsButton, deletePointsButton, copyAndPastePointEdgeButton;
	private JTextField blankImageWidth, blankImageHeight, rectRowSizeField, rectColSizeField, triFirstRowField,
			triLastRowField, nextPointIdField;
	...
	public ChessBoardPanel(EventCallback eventCallback) {
		...
		scrollPane = new JScrollPane(toolAreaPanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	private void setupToolAreaPanel() {
		...
		helpHotKeyLabel.setToolTipText("<html>Any mouse action on the chess board activates the<br>"
				+ "hot key edit function.<br>" + "a key - add points<br>" + "d key - delete selected points<br>"
				+ "c key - copy and paste points<br>" + "e key - create edge for selected points<br>"
	...
	public void setupEventListeners() {
		...
		scalePointCoordinatesImageRadio
				.addActionListener(getRadioButtonActionListener(scalePointCoordinatesImageRadio));
		scalePointCoordinatesPanelRadio
				.addActionListener(getRadioButtonActionListener(scalePointCoordinatesPanelRadio));
	...
	public void setupPointUpdateSizeMethodPanel() {
		addToPanel(toolAreaPanel, new JLabel("<html><br>Point scaling method</html>"), GridBagConstraints.CENTER,
		...
		scalePointCoordinatesImageRadio = new JRadioButton(
				"<html>Scale only coordinates<br>by chess board image</html>");
		...
		addToPanel(pointScaleOptionPanel, scalePointCoordinatesImageRadio, GridBagConstraints.WEST,
				GridBagConstraints.NONE);
		addToPanel(pointScaleOptionPanel, scalePointCoordinatesPanelRadio, GridBagConstraints.WEST,
				GridBagConstraints.NONE);
	...
	private void setupAddPointPanel() {
		...
		addPointsButton.setToolTipText(
				"<html>If single point is selected, the point will be directly added to the chess board;<br>"
		...
		deletePointsButton.setToolTipText(
				"Delete all the selected points. All edges connected to those points will also be removed");
		...
		addToPanel(toolAreaPanel, new JLabel("<html><br>Piece placing points</html>"), GridBagConstraints.CENTER,
				GridBagConstraints.NONE);
	...
	private void setupEdgeDirectionSettingPanel() {
		...
		addToPanel(toolAreaPanel, new JLabel("<html><br>Edge direction between points</html>"),
		...
		createEdgeForPointsButton
				.setToolTipText("<html>Create edge for the selected points in between each neighbour point,<br>"
		...
		if (jb == browseBoardImageButton)
			...
			return new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (indices.length > 0) {
						int index = (indices[0] < edgeDirectionList.getModel().getSize() ? indices[0]
								: edgeDirectionList.getModel().getSize() - 1);
	...
	private ActionListener getRadioButtonActionListener(JRadioButton jrb) {
		if (jrb == imageFromFileRadio)
			return new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (imageFile != null)
						graphicAreaPanel.setBoardImage(imageFile);
				}
			};
		...
		else if (jrb == scalePointNoneRadio || jrb == scalePointAllImageRadio || jrb == scalePointAllPanelRadio
				|| jrb == scalePointCoordinatesImageRadio || jrb == scalePointCoordinatesPanelRadio)
			return new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
	...
	private void setBlankImage() {
		...
		try {
			if (!graphicAreaPanel.setBoardImage(Integer.parseInt(blankImageWidth.getText()),
					Integer.parseInt(blankImageHeight.getText())))
				throw new Exception();
		} catch (Exception e) {
	...
	public void addEdgeDirection(String s) {
		...
		for (int i = 0; i < listModel.getSize(); i++)
			if (s.equals(listModel.getElementAt(i)))
				return;
	...
	public void createEdgeForPointsButtonAction() {
		...
		if (selectedIndices.length > 0)
			graphicAreaPanel.createEdgesForSelectedPoints(
					((DefaultListModel<String>) edgeDirectionList.getModel()).getElementAt(selectedIndices[0]));
		else
			JOptionPane.showMessageDialog(UIHandler.getMainWindow(), "No selected edge direction.", "Error - Chreator",
	...
	public String getBoardImage() {
		if (imageFromFileRadio.isSelected() && imageFile != null) {
			...
			BufferedImage bImage = new BufferedImage(Integer.parseInt(blankImageWidth.getText()),
					Integer.parseInt(blankImageHeight.getText()), BufferedImage.TYPE_INT_ARGB);
			Graphics2D graphics = bImage.createGraphics();

			graphics.setColor(Color.WHITE);
			graphics.fillRect(0, 0, Integer.parseInt(blankImageWidth.getText()),
					Integer.parseInt(blankImageHeight.getText()));
			graphics.dispose();
			try {
				ImageIO.write(bImage, "png", boardPicF);
			} catch (Exception e) {
